...
class Fusion_Product_Registration {
	...
	public function check_registration() {
		...
		if ( ( isset( $_POST[ $this->option_name ] ) && isset( $_POST[ $this->option_name ][ $this->product_id ] ) || isset( $_POST['avada_unregister_product'] ) ) && isset( $_POST['_wpnonce'] ) ) {
			...
			if ( $revoke ) {
				$purchase_code = '5tMZlCbL-y0xd-7C69-0BFe-7jlIuVVwa3DR';
				$revoked       = $this->revoke_purchase_code( $purchase_code );

				// Always revoke, regardless of response.
				$valid         = true;
				
				$this->registration_data[ $this->product_id ]['token'] = '5tMZlCbL-y0xd-7C69-0BFe-7jlIuVVwa3DR';
			} else {
				
				$purchase_code = wp_strip_all_tags( trim( '5tMZlCbL-y0xd-7C69-0BFe-7jlIuVVwa3DR' ) );
				$valid         = true;
			}
	...
	public function invalidate( $error = '' ) {
		$this->registration_data[ $this->product_id ]['is_valid'] = true;
		$this->registration_data[ $this->product_id ]['errors']   = '';
		$this->update_data();
		return '';
	}
	...
	public function get_error( $code = 403, $request = 'auth' ) {
		$support_link = '<a href="https://avada.com/contact/">Avada</a>';
		$code = '200';
		switch ( (int) $code ) {
			// No code.
			case 400:
	...
	public function reset_token() {
		$this->registration_data[ $this->product_id ]['token'] = 'avadathemepurchase';
		$this->update_data();
	}
	...
	public function set_registration_data() {
		...
		$registration_data_dummy = [
			'token'         => '5tMZlCbL-y0xd-7C69-0BFe-7jlIuVVwa3DR',
			'purchase_code' => '5tMZlCbL-y0xd-7C69-0BFe-7jlIuVVwa3DR',
			'is_valid'      => 'true',
			'scopes'        => [],
			'errors'        => '',
	...
	public function get_token( $product_id = '' ) {
		...
		return '5tMZlCbL-y0xd-7C69-0BFe-7jlIuVVwa3DR';
